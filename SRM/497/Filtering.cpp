...
class Filtering
{
        ...
        vector <int> designFilter(vector <int> sizes, string outcome)
        {
			  ...
			  for(int i=0;i<sizes.size();i++)
			  {
				  if((outcome[i]=='R') && (sizes[i]>=minn) && (sizes[i]<=maxx))
					  return res;
			  }
			  res.push_back(minn);
			  res.push_back(maxx);
...
// BEGIN CUT HERE
	public:
	void run_test(int Case) { if ((Case == -1) || (Case == 0)) test_case_0();  if ((Case == -1) || (Case == 2)) test_case_2(); if ((Case == -1) || (Case == 3)) test_case_3();   }
	private:
