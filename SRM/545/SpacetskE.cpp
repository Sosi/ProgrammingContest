        ...
        int countsets(int L, int H, int K)
        {
            ...
            for(int i=1; i<=L; i++)
            {
                for(int j=1; j<=H; j++)
                {
                    ...
                    {
                        if(H/j+1 < K) continue;
                        for(int k = (K-1)*i; k<(L/i)*i; k+=i)
                        {
                            ret= (ret+ i*C[min(k/i+1, H/j+1)][K]*2LL) %MOD;
                        }
                        ret= (ret+ (L-(L/i*i)+1)*C[min(L/i+1, H/j+1)][K]*2LL) %MOD;
                    }
                }
            }
            return ret;
