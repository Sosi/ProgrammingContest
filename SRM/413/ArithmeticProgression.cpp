...
// END CUT HERE
#line 5 "ArithmeticProgression.cpp"
...
class ArithmeticProgression
{
        ...
        double minCommonDifference(int a0, vector <int> seq)
        {
				 			 double Ma = 0;
				 			 double Mi = 1e10;
				 			 for(int i=0; i<seq.size(); i++)seq[i]-=a0;
				 			 for(int i=0; i<seq.size(); i++) Ma = max(Ma,seq[i]*1.0/(i+1.0));
				 			 for(int i=0; i<seq.size(); i++) Mi = min(Mi,(seq[i]+1.0)/(i+1.0));
				 			 if(Ma>=Mi) return -1;
				 			 else return Ma;
        }
