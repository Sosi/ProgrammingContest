...
class BlockDistance
{
    public:
        ...
        {
            string oldT, newT;
            for(int i=0; i<oldText.size(); i++) oldT += oldText[i];
            for(int i=0; i<newText.size(); i++) newT += newText[i];
            int m = oldT.size();  int n= newT.size();
            for(int i=0; i<= m; i++)
            {
                for(int j=0; j<= n ; j++)
                {
            ...
            for(int i=0; i<=m ; i++)
            {
                for(int j=0; j<= n; j++)
                {
                    if(i< m  && j< n && oldT[i] == newT[j])
                    {
                        dp[i+1][j+1][0] = min(min(dp[i][j][0], dp[i][j][1]) ,dp[i+1][j+1][0]);
                    }
                    if(j<n) dp[i][j+1][1] = min(min(dp[i][j][1], dp[i][j][0]+1), dp[i][j+1][1]);
                }
            }
            cout<<dp[oldT.size()][newT.size()][0] <<"   "<< dp[oldT.size()][newT.size()][1]<<endl;
            int ret =min(dp[oldT.size()][newT.size()][0], dp[oldT.size()][newT.size()][1]);
