...
class Taxi
{
        ...
        double maxDis(int maxX, int maxY, int taxiDis)
        {
            int X = min(maxX, maxY);
            int Y = max(maxX, maxY);
            if( X + Y < taxiDis) return -1;
            else if( taxiDis > Y-1) return sqrt(1.0*Y*Y+1.0*(taxiDis - Y)*(taxiDis- Y));else if(taxiDis <= Y) return taxiDis; else  return sqrt(1.0*(taxiDis-1 )* (taxiDis -1)+1.0);
        }
