...
{
    public:
        string dates(string circle, int kk)
        {
            bool vis[55];
            memset(vis, 0 ,sizeof(vis));
            map<int, int> M;
            REP(i,26)  M[i+'a'] = 0,M[i+'A']=0;
            int X = 0; int Y = 0;
            REP(i, circle.size())
            {
                if(circle[i] >= 'A' && circle[i]<='Z') X++;else if(circle[i] >= 'a' && circle[i] <= 'z') Y ++;
                M[circle[i]]=1;
            }
            string ret="";
            int id = 0;
            while(X>0 && Y> 0)
            {
                ...
                int cur = (id+circle.size()-1)%circle.size();
                cout<<"! "<<cur<<"  id  "<<id<<endl;
                vis[cur]=1; M[circle[cur]] = 0;
                if(circle[cur] >= 'A' && circle[cur] <= 'Z')
                {
                    REP(j,26) if(M['a'+j]) 
                    {
                        M['a'+j] = 0;
                        REP(k,circle.size()) if(circle[k] == 'a'+j) vis[k] = 1;
                        ret += circle[cur];
                        ret += (char)('a'+j);
                        break;
                    }
