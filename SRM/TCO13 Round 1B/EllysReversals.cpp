...
bool equal(string A, string B)
{
    ...
    if(A.size() %2 ==1 && A[A.size()-1] != B[B.size()-1]) return 0;
    multiset<string> a;
    multiset<string> b;
    for(int i=0; i<A.size()/2*2; i+=2)
    {
        string tmp = "";
        if(A[i] < A[i+1]) tmp+= A[i], tmp+= A[i+1];
        else tmp+= A[i+1],tmp+= A[i];
        a.insert(tmp);
        tmp = "";
        if(B[i] < B[i+1]) tmp+= B[i], tmp+= B[i+1];
        else tmp+= B[i+1], tmp+= B[i];
        b.insert(tmp);
    }
    return a == b;
}
class EllysReversals
{
        ...
        int getMin(vector <string> words)
        {
            ...
            REP(k,words.size())
            {
                ...
                {
                    if(!vis[j] && equal(words[k], words[j]))
                    {
                        cout<<words[k]<< " "<<words[j]<<endl;
                        vis[k] = 1;
                        vis[j] = 1;
                        break;
                    }
