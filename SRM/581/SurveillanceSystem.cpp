...
class SurveillanceSystem
{
        ...
        string getContainerInfo(string containers, vector <int> reports, int L)
        {
            ...
            string ret="";
            REP(i,containers.size()) ret+="-";
            REP(i,containers.size())
            {
                if(i+L <= containers.size())
                {
                    int tmp = 0;
                    REP(j,L)
                    {
                        if(containers[j+i]=='X') tmp++;
                    }
                    num[tmp]++;
                }
            }
            REP(i,reports.size()) src[reports[i]]++;
            REP(i,containers.size())
            {
                if(i+L <= containers.size())
                {
                    int tmp=0;
                    REP(j,L) if(containers[i+j] == 'X') tmp++;
                    if(num[tmp] == src[tmp])
                    {
                        REP(j,L) ret[i+j] = '+';
                    }else
            ...
            REP(k,ret.size())
            {
                if(ret[k]=='?')
                {
                    ...
                    REP(i,ret.size())
                    {
                        if(i+L <= containers.size() && ((k< i)|| (k>=i+L)))
                        {
                            int tmp = 0;
                            REP(j,L) if(containers[i+j]  =='X') tmp++;
                            zero[tmp]++;
                    ...
                    REP(i,55) if(zero[i] < src[i]) ret[k]='+';
                }
            }
            return ret;
        }
