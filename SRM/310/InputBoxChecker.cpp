...
class InputBoxChecker
{
        ...
        vector <string> checkPrefix(int smallest, int largest, vector <int> numbers)
        {
            ...
            {
                if(numbers[i] > largest) ret.push_back("INVALID");
                else if(numbers[i]>=smallest && numbers[i] <= largest) ret.push_back("VALID");
                else
                {
                    long long small = numbers[i]; long long large = numbers[i];
                    small *=10; large*=10; large+=9;
                    bool flag = 0;
                    while(small <= largest)
                    {
                        if(small <= largest && large >= smallest) 
                        {
                            ret.push_back("VALID");
                            flag = true;
                            break;
