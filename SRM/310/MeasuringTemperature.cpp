...
{
        public:
        double averageTemperature(vector <int> mea)
        {
            int num = 0;
            double sum = 0;
            int n= mea.size();
            for(int i=0; i<n; i++)
            {
                if(mea[i]<-273) continue;
                int tempnum=0; int temp=0;
                for(int j=-2; j<=2; j++)
                {
                    if(j==0) continue;
                    if(i+j >=0 && i+j <n)
                    {
                        tempnum++;
                        if(abs(mea[i]- mea[i+j]) >2) temp++;
                    }
                }
                if(i==n-1) cout<<"!"<<tempnum<<" "<<temp<<endl;
                if(tempnum == temp) continue;
                cout<<mea[i]<<" ";
                num++;
                sum += mea[i];
            }
            cout<<endl;
            cout<<sum<<endl; cout<<num<<endl;
            if(num == 0) return -300.0f;
            else return sum/num;
        }
