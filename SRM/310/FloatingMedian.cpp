...
class FloatingMedian
{
    public:
        long long sumOfMedians(int seed, int mul, int add, int N, int K)
        {
            ...
            for(int i=1; i<N; i++)
            {
                C.push_back((C[i-1]*mul + add) % MOD);
            }
            long long res = 0LL;
            multiset<long long > lowerH, upperH;
            for(int i=0; i<N; i++)
            {

                // add the new temperature

                if (lowerH.size()==(K+1)/2 && getLastElement(lowerH) <= C[i])
                    upperH.insert( C[i] );
                else
                    lowerH.insert( C[i] );
                ...
                if (i >= K) {
                    if (C[i-K] <= getLastElement( lowerH ))
                        lowerH.erase(lowerH.find( C[i-K] ));
                    else
                        upperH.erase(upperH.find(C[i-K] ));
                }

                // update the counts of elements in our multisets

                while (lowerH.size() > (K+1)/2)
                    upperH.insert( popLastElement( lowerH ));
                while (upperH.size() > K/2)
                    lowerH.insert( popFirstElement( upperH ));

                // compute the median

                if (i >= K-1) res += getLastElement( lowerH );
            }
