        ...
        int transform(vector <string> board1, vector <string> board2)
        {
			...
			for(int i=0;i<board1[0].size();i++)
			{
				for(int j=0;j<board1.size();j++)
				{
					...
					{
						temp.x=j,temp.y=i;
						board1P.push_back(temp);
					}
					if(board1[i][j]=='N')
					{
						temp.x=j,temp.y=i;
						board1N.push_back(temp);
					}
				}
			}
			for(int i=0;i<board2[0].size();i++)
			{
				for(int j=0;j<board2.size();j++)
				{
					...
					{
						temp.x=j,temp.y=i;
						board2P.push_back(temp);
					}
					if(board2[i][j]=='N')
					{
						temp.x=j,temp.y=i;
						board2N.push_back(temp);
					}
				}
			}
			if(board1P.size()<board2P.size())
				return -1;

			//下面这一部判断P是否合法
			for(int j=0;j<board2P.size();j++)
			{
				for(int i=0;i<board1P.size();i++)
				{
					if(board1P[i].y<board2P[j].y)
						continue;
					if(board1P[i].y==board2P[j].y)

				}
			}
			//下面使用O(N^3)算法，最小需要的是几步操作，每步小于一个值。。
