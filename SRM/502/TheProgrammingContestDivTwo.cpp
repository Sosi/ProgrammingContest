...
class TheProgrammingContestDivTwo
{
		...
		//	do
			//{
				...
				if(solved>ressolved)
					ressolved=solved,respen=penalty;
				if(solved==ressolved)
					ressolved=solved,respen=min(penalty,respen);
		//	}while( next_permutation(requiredTime.begin(),requiredTime.end()) );
			vector <int> p;
			p.push_back(ressolved);
			p.push_back(respen);
			return p;
		}
