...
class XorTravelingSalesman
{
        ...
        int maxProfit(vector <int> cityValues, vector <string> roads)
        {
            ...
            while(!Q.empty())
            {
                ...
                for(int i=0; i<n; i++)
                {
                    if(i!=tmp.first && roads[tmp.first][i]=='Y' )
                    {
                        if(!vis[i][cityValues[i]^tmp.second])
                        {
                            ret = max(ret, cityValues[i] ^ tmp.second);
                            vis[i][cityValues[i]^tmp.second] = 1;
                            Q.push(make_pair(i, cityValues[i]^tmp.second));
                        }
                    }
                }
            }
