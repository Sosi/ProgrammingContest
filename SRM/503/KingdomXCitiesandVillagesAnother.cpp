...
// END CUT HERE
#line 5 "KingdomXCitiesandVillagesAnother.cpp"
...
class KingdomXCitiesandVillagesAnother
{
        ...
        double determineLength(vector <int> cityX, vector <int> cityY, vector <int> villageX, vector <int> villageY)
        {
			...
			{
				double mm=dis(City[0],Village[0]);
				int flaga=0;
				int flagb=0;
				for(int i=0;i<Village.size();i++)
				{
					...
					{
						if(dis(Village[i],City[j])<=mm)
						{
							mm=dis(Village[i],City[j]);
							flaga=i;
							flagb=j;
						}
					}
				}
				res+=dis(Village[flaga],City[flagb]);
				City.push_back(Village[flaga]);
				Village.erase(Village.begin()+flaga);
			}
			return res;
