...
bool possible(vector<int> X, vector<int > Y, vector<int> R, double dis)
{
    bool ret = true;
    for(int i=0; i<X.size(); i++)
    {
        double d = sqrt(1.0*X[i]*X[i]+1.0*Y[i]*Y[i]);
        if(dis > d +R[i] || dis < d-R[i]) continue;
        else
        {
            ret = false;
            break;
...
double RadiusProbability(vector <int> X, vector <int> Y, vector <int> R, int Z)
{
    ...
    for(int i=0; i<X.size(); i++)
    {
        ...
        if(dis -R[i] > 0)
        ret.push_back( dis - R[i] );
        ret.push_back( dis + R[i]);
    }
    ret.push_back(0.0f);
    ret.push_back(Z*1.0);
    ret.push_back(1e9+10.0);
    ...
    {
        if(ret[i] == Z *1.0) break;
        if(ret[i] == ret[i+1]) continue;
        else
        {
            double pick = (ret[i]+ret[i+1])/2.0;
            if(possible(X,Y,R,pick))
            {
                ans += ret[i+1] - ret[i];
            }
        }
