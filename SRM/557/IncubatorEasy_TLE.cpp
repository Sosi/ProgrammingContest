...
class IncubatorEasy
{
    public:
        int maxMagicalGirls(vector <string> love)
        {
            ...
            int ret = 0;
            do{
                REP(i,N) {Magic[i] = 0; Pro[i] = 0;}
                REP(i,N)
                {
                    if(Magic[C[i]] == 0 && Pro[C[i]] == 0)
                    {
                        Magic[C[i]] = 1;
                        dfs(C[i]);
                    }
                    int num = 0;
                    REP(k,N) if(Magic[C[k]] && Pro[C[k]] == 0) num++;
                    ret = max(ret, num);
                }
                //REP(i,N) cout<<C[i]<<" "; cout<<endl;
                //REP(i,N) cout<<Magic[C[i]]<<" "; cout<<endl;
                //REP(i,N) cout<<Pro[C[i]]<<" "; cout<<endl;
                //int num = 0;
                //REP(i,N) if(Magic[C[i]] && Pro[C[i]] == 0) num++;
                //ret = max(ret, num);
