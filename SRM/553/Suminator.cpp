...
long long process(vector<int> pro)
{
    stack<long long> A;
    for(int i=0; i<pro.size(); i++)
    {
        if(pro[i] == 0)
        {
            long long X = 0LL; long long Y =0LL;
            if(A.empty());
            else {X=A.top(),A.pop();}
            if(A.empty());
            else {Y = A.top(); A.pop();}
...
class Suminator
{
        ...
        int findMissing(vector <int> program, int wantedResult)
        {
            for(int i=0; i<program.size(); i++)
            {
                ...
                {
                    program[i]=0;
                    if(process(program) == wantedResult) return 0;
                    cout<<"0____"<<process(program)<<endl;
                    program[i]=1;
                    long long X = process(program);
                    program[i]=2;
                    long long Y = process(program);
                    if(X == Y || X> wantedResult ) return -1;
                    else return wantedResult - X+1;
                }
            }
        }
