...
class EllysThreeRivers
{
    public:
        ...
        {
            int M= width.size()+1;
            for(int i=0; i<55; i++) for(int j=0; j<100005; j++) dp[i][j]=100000000;
            ...
            for(int i=1; i<M; i++)
            {
                for(int j=0; j<=length; j++)
                {
                    for(int k=0; k<=j; k++)
                    {
                        dp[i][j]= min(dp[i][j],dp[i-1][k] + dis(width[i-1],j-k)/speed[i-1] );
                    }
                }
            }
            return dp[M-1][length];
