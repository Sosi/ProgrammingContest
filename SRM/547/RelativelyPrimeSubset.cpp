...
// END CUT HERE
#line 5 "RelativelyPrimeSubset.cpp"
...
class RelativelyPrimeSubset
{
        ...
        int findSize(vector <int> S)
        {
            ...
            int n = S.size();
            int ret = 0;
            bool vis[55]; fill(vis, vis+55, 0);
            for(int i=0; i<prime.size(); i++)
            {
                bool flag = 0;
                for(int j=0; j<n; j++)
                {
                    if(S[j]%prime[i]==0 && vis[j]==0) 
                    {
                        vis[j]=1;
                        flag = 1;
                    }
...
// BEGIN CUT HERE
	...
	void test_case_1() { int Arr0[] = {1,7,14,49}; vector <int> Arg0(Arr0, Arr0 + (sizeof(Arr0) / sizeof(Arr0[0]))); int Arg1 = 2; verify_case(1, Arg1, findSize(Arg0)); }
