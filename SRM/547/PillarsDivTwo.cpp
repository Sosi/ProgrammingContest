...
class PillarsDivTwo
{
        ...
        double maximalLength(vector <int> height, int w)
        {
            ...
            for(int i=1; i<n; i++)
            {
                dp[i][0]=max(dp[i-1][1]+hypot(w, height[i-1]-1), dp[i-1][0]+hypot(w,0));
                dp[i][1]+=max(dp[i-1][1]+hypot(w, height[i]-height[i-1]), dp[i-1][0]+hypot(height[i]-1,w));
            }
