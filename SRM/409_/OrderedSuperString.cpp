    ...
    public:
        int getLength(vector <string> words)
        {
            ...
            for(int i=1;i<words.size(); i++)
            {
                ...
                {
                    bool flag=0;
                    for(int j=ind; j<tag.size()&& j-ind<words[i].size(); j++)
                    {
                        if(tag[j]!=words[i][j-ind]){ flag=1; ind++; break;}
                    }
                    if(flag==0)
                    {
                        if(tag.size()-ind < words[i].size())
                        {
                            for(int k=tag.size()-ind; k<words[i].size();k++)
                                tag+=words[i][k];
                        }
                        break;
                    }
                }
                if(ind==tag.size()) tag+=words[i];
            }
            cout<<tag<<endl;
            return tag.size();
