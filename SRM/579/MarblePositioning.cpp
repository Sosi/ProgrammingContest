...
class MarblePositioning
{
        ...
        double totalWidth(vector <int> r)
        {
            ...
            REP(i,r.size()) radius.push_back(1.0*r[i]);
            sort(radius.begin(), radius.end());
            double ret = 2*accumulate(radius.begin(), radius.end(), 0.0);
            do{
                REP(i,radius.size()) cout<<radius[i]<<" "; cout<<endl;
                REP(i,radius.size()) REP(j,radius.size())
                {
                    if(i==j) dp[i][i] = 0.0;
                    else
                        dp[i][j] = 2*sqrt(radius[i]*radius[j]);
                }
                REP(k,radius.size()) REP(i,radius.size()) REP(j,radius.size())
                {
                    if(j<i || k<i || k> j) continue;
                    dp[i][j] = max(dp[i][j], dp[i][k]+dp[k][j]);
                }
                ret = min(dp[0][radius.size()-1], ret);
...
// BEGIN CUT HERE
	...
	void test_case_0() { int Arr0[] = {519721, }; vector <int> Arg0(Arr0, Arr0 + (sizeof(Arr0) / sizeof(Arr0[0]))); double Arg1 = 2.8284271247461903; verify_case(0, Arg1, totalWidth(Arg0)); }
