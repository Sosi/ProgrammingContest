...
class PizzaDivision
{
    public:
        int howMany(vector <string> toppings)
        {
            ...
            for(int i=0; i<n; i++)
            {
                sscanf(toppings[i].c_str(), "%d%d", &E[i][0], &E[i][1]);
            }
            if(n==1)
            {
                if(E[0][0]==0 && E[0][1]==0) return -1;
                else return 1;
            }
            int ret=0;
            ...
            for(int i=0; i<n; i++)
            {
                for(int j=i+1; j<n; j++)
                {
                    ...
                    int x,y;
                    if(E[i][0]*(E[i][1]-E[j][1]) - E[i][1]*(E[i][0]-E[j][0]) ==0){ y = E[j][1]- E[i][1], x=E[j][0]-E[i][0]; C.push_back(make_pair(x,y));}
                    if((E[i][1] + E[j][1])*(E[i][1]-E[j][1]) + (E[i][0]+E[j][0])*(E[i][0]-E[j][0]) == 0) { y=-(E[j][0]-E[i][0]), x=E[j][1]-E[i][1]; C.push_back(make_pair(x,y));}
                    for(int m=0; m<C.size(); m++)
                    {
                        x=C[m].first; y=C[m].second;
                        int num=0;
                        for(int p=0; p<n; p++)
                        {
                            if(p==i || p==j) continue;
                            ...
                            for(int q=0; q<n; q++)
                            {
                                if(q==i || q==j || p== q) continue;
                                if( (E[p][1]-E[q][1])*y+(E[p][0]-E[q][0])*x==0 && x*(E[p][1]+E[q][1])==y*(E[p][0]+E[q][0]) ) { flag=0; break;}
                            }
                            if(flag == 0) num++;
            ...
            bool vis[2500]; memset(vis, 0, sizeof(vis));
            for(int i=0; i<D.size(); i++)
            {
                if(!vis[i])
                {
                    ...
                    for(int j=i+1; j<D.size(); j++)
                    {
                        if(!vis[j])
                        {
                            if(D[i].first * D[j].second - D[j].first * D[i].second == 0) vis[j]=1;
                        }
                    }
                }
            }
