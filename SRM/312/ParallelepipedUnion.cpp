...
// END CUT HERE
#line 5 "ParallelepipedUnion.cpp"
...
class ParallelepipedUnion
{
        ...
        int getVolume(vector <string> parallelepipeds)
        {
            int a[6],b[6];
            int ret=0;
            sscanf(parallelepipeds[0].c_str(), "%d%d%d%d%d%d", &a[0], &a[1], &a[2], &a[3], &a[4], &a[5]);
            sscanf(parallelepipeds[1].c_str(), "%d%d%d%d%d%d", &b[0], &b[1], &b[2], &b[3], &b[4], &b[5]);
            for(int i=1; i<=100; i++)
            {
                for(int j=1; j<=100; j++)
                {
                    for(int k=0; k<=100; k++)
                    {
                        if( i>=a[0]&& i<a[3] && j>=a[1] && j< a[4] && k>=a[2] && k<a[5] ||
                            i>=b[0]&& i<b[3] && j>=b[1] && j< b[4] && k>=b[2] && k<b[5])
                            ret++;
                    }
                }
            }
