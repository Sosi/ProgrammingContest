...
class TreasuresPacking
{
        ...
        double maximizeCost(vector <string> treasures, int W)
        {
            ...
            for(int i=0; i<=W; i++) for(int j=0; j<=n; j++) dp[i][j]= 0.0f;
            double ret = 0.0f;
            for(int i=0; i<n; i++)
            {
                istringstream sin(treasures[i]);
                int a,b; sin>>a>>b;
                char c; sin>>c;
                if(c == 'N')
                {
                    for(int j = 0; j<=W; j++)  // weight
                    {
                        if(j + a <= W)
                        {
                            dp[j+a][i+1]= max(max(dp[j+a][i], dp[j][i] + b), dp[j+a][i+1]);
                            ret = max(ret, dp[j+a][i+1]);
                        }
                    }
                }
                else
                {
                    for(int j=0; j<=W; j++)
                    {
                        for(int k=1; k<=a; k++)
                        {
                            if(j+k<=W)
                            {
                                dp[j+k][i+1] = max(max(dp[j+k][i], dp[j][i] + b*1.0/a*k ), dp[j+k][i+1]);
                                ret = max(ret, dp[j+k][i+1]);
                            }
                        }
                    }
                }
