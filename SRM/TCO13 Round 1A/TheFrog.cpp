...
class TheFrog
{
        ...
        double getMinimum(int D, vector <int> L, vector <int> R)
        {
            double minDis = 0;
            double ret = D;
            REP(i,L.size()) minDis = max(abs(1.0*R[i]-L[i]),minDis);
            cout<<"Mindis "<<minDis<<endl;
            REP(i,R.size()) for(int t= 1; t<= D; t++)
            {
                double dis = (double)R[i]/t;
                if(dis > ret) continue;
                if(dis < minDis) break;
                bool good = 1;
                REP(j,L.size()){
                    if(R[j]*t%R[i] == 0 && R[j]*t/R[i]- 1 == L[j]*t/R[i]) continue;
                    else if (t*R[j]/R[i]> L[j]*t/R[i]) good = 0;    
                }
                if(good) ret = min(ret, dis);
            }
            return ret;
        }
