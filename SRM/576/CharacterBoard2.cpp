        ...
        int countGenerators(vector <string> fragment, int W, int i0, int j0)
        {
            ...
            for(int w = 1; w<= W; w++)
            {
                ...
                for(int i=0; i<fragment.size(); i++)
                {
                    ...
                    {
                        int idx = ((i+i0)* W+ j+j0) % w;
                        if(c[idx] != '?' && c[idx] != fragment[i][j]) ok = 0;
                        else if(c[idx] == '?') c[idx] = fragment[i][j];
                    }
                }
                if(ok == 0) continue;
                long long res = 1LL;
                for(int i=0; i<w; i++) if(c[i]=='?') res = (res * 26LL)%MOD;
                ret = (ret + res )% MOD;
            }
            return (int)ret;
        }
