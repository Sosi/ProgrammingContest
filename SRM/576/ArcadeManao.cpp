...
class ArcadeManao
{
        ...
        int shortestLadder(vector <string> level, int coinRow, int coinColumn)
        {
            ...
            int columnnum = level[0].size();
            for(int L = 0; L<=54; L++)
            {
                memset(vis, 0,sizeof(vis));
                queue<pair<int,int> > Q;
                Q.push(make_pair(rownum,1));
                vis[rownum][1] = 1;
                while(!Q.empty())
                {
                    pair<int,int> C = Q.front();
                    if(C.first == coinRow && C.second == coinColumn) return L;
                    Q.pop();
                    for(int x= -L; x<=L; x++)
                    {
                        if( x!=0 && C.first + x >=1 && C.first + x <= rownum && !vis[C.first+x][C.second]&& level[C.first+x-1][C.second-1] == 'X')
                        {
                            Q.push(make_pair(C.first+x,C.second));
                            cout<<L<<" "<<C.first+x<<" "<<C.second<<endl;
                            vis[C.first+x][C.second] =1;
                        }
                    }
                    for(int x = -1; x <=1; x++)
                    {
                        if(x!= 0 && C.second + x >=1 && C.second +x <= columnnum && !vis[C.first][C.second+x] && level[C.first-1][C.second+x-1] == 'X')
                        {
                            Q.push(make_pair(C.first, C.second+x));
                            cout<<L<<"_ "<<C.first<<" "<<C.second+x<<endl;
                            vis[C.first][C.second+x] = 1;
                        }
                    }
                }
            }
