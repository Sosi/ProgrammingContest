...
class GreedyTravelingSalesman
{
        ...
        int worstDistance(vector <string> thousands, vector <string> hundreds, vector <string> tens, vector <string> ones)
        {
            ...
            for(int i=0; i<n ; i++)
            {
                for(int j=0; j<thousands[0].size(); j++)
                {
                    E[i][j]=(thousands[i][j]-'0')*1000+ (hundreds[i][j]-'0')*100+ (tens[i][j]-'0')*10 + ones[i][j]-'0';
                }
            }
            int ans  =0;
            for(int i=0; i<n; i++)
            {
                for(int j=0; j<n; j++)
                {
                    if(i==j) continue;
                    vector<int> res;
                    res.push_back(1); res.push_back(9999);
                    for(int k=0; k<n; k++)
                    {
                        if(i==k) continue;
                        if(E[i][k]>1) res.push_back(E[i][k]-1);
                        if(E[i][k]<9999) res.push_back(E[i][k]+1);
                        res.push_back(E[i][k]);
                    }
                    ...
                    for(int k=0; k<res.size(); k++)
                    {
                        ...
                        for(int a = 1; a<n; a++)
                        {
                            int tempedge = 1000000; int id = -1;
                            for(int b = 0; b<n; b++)
                            {
                                if(!vis[b] && E[cur][b] < tempedge)
                                {
                                    tempedge = E[cur][b];
                                    id = b;
                                }
                            }
                            //cout<<"ID "<<id<<" tempedge  "<<tempedge;
                            vis[id] = 1;
                            cur = id;
                            tempans += tempedge;
                        }
