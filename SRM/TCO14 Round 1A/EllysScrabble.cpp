...
string getMin(string letters, int maxDistance)
{
	...
	{
		S.push_back( make_pair(letters[i], i));
	}

	string ret;

	vector<pair<char, int> > tmp;
	for(int i=0; i<  min(letters.size() *1.0,maxDistance + 1.0); i++) tmp.push_back(make_pair(letters[i], i));
	for(int i=0; i<letters.size(); i++)
	{
		// search
		...
		// push
		if(i + maxDistance + 1 < letters.size())
		{
			tmp.push_back(S[ i+ maxDistance +1]);
		}
	}
	return ret;
}
