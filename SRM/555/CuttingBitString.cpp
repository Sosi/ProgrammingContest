...
void init()
{
    ...
    while(1)
    {
        long long q = p;
        vector<int> ls;
        while(q>0) ls.push_back(q%2LL),q/=2LL;
        if(ls.size()>50) break;
        string s;
        for(int i=ls.size()-1; i>=0; i--) if(ls[i]) s+='1';else s+='0';
        C.push_back(s);
        p=p*5LL; 
    }
}
class CuttingBitString
{
        ...
        int getmin(string S)
        {
            ...
            while(!Q.empty())
            {
                ...
                {
                    if(C[i].size() <= temp.first.size() &&  C[i] == temp.first.substr(0,C[i].size()))
                    {
                        if(temp.first.substr(C[i].size(),temp.first.size()- C[i].size())=="") return temp.second;
                        else 
                            Q.push(make_pair(temp.first.substr(C[i].size(),temp.first.size()-C[i].size()),temp.second+1));
                    }
                }
            }
            return -1;
