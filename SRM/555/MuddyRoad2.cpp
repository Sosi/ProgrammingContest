        ...
        int theCount(int N, int muddyCount)
        {
            ...
            for(int i=1; i<=N; i++)
            {
                ...
                for(int j=2; j<=i; j++)
                {
                    for(int c=1; c<=2; c++)
                    {
                        f[i][j][c]=(f[i-c][j-1][1]+f[i-c][j-1][2])%MOD;
                        if(i>=3) f[i][j][c]=(f[i][j][c]+f[i-3][j][c])%MOD;
                    }
                }
            }
            int ret=c[N-2][muddyCount]-f[N-muddyCount][muddyCount+1][1]-f[N-muddyCount][muddyCount+1][2];
            ret%=MOD;
            ret=(ret+MOD)%MOD;
            return ret;
