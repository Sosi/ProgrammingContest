...
class XorBoardDivTwo
{
        ...
        int theMax(vector <string> board)
        {
            ...
            vector<int> col(board[0].size(), 0);
            int num=0;
            for(int i=0; i<board.size(); i++)
            {
                for(int j=0; j<board[0].size(); j++)
                    if(board[i][j]=='1') row[i]++;
                num+=row[i];
            }
            for(int i=0; i<board[0].size(); i++)
            {
                for(int j=0; j<board.size(); j++)
                    if(board[j][i]=='1') col[i]++;
            }
            int ret=0;
            for(int i=0; i<row.size(); i++)
            {
                ...
                {
                    ret = max(ret, num - row[i] -col[j] + (int)row.size() -row[i]+(int)col.size()-col[j]+(board[i][j]=='1'?2:-2));
                }
            }
            return ret;
        }
