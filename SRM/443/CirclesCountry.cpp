...
class CirclesCountry
{
        ...
        int leastBorders(vector <int> X, vector <int> Y, vector <int> R, int x1, int y1, int x2, int y2)
        {
            ...
            {
                if((X[i]-x1)*(X[i]-x1)+ (Y[i]-y1)*(Y[i]-y1)<R[i]*R[i] && (X[i]-x2)*(X[i]-x2)+ (Y[i]-y2)*(Y[i]-y2) > R[i]*R[i]) ret++;
                else if((X[i]-x1)*(X[i]-x1)+ (Y[i]-y1)*(Y[i]-y1) > R[i]*R[i] && (X[i]-x2)*(X[i]-x2) + (Y[i]-y2)*(Y[i]-y2) < R[i]*R[i]) ret++;
            }
            return ret;
        }
