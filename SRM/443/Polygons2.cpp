...
//End Sosi TopCoder
// END CUT HERE
long long dp[11][50060];
int LIM = 50055;
class Polygons2
{
        public:
        long long number(vector <int> seg, int K)
        {
            int N = seg.size();
            long long ret = 0;
            sort(seg.begin(), seg.end());
            for(int i=0; i<10; i++) for(int j=0; j<LIM; j++) dp[i][j]=0LL;
            dp[0][0]=1;
            for(int i=1; i<N; i++)
            {
                for(int k=K; k>0;k--)
                {
                    for(int s=0; s<=LIM; s++)
                    {
                        dp[k][min(s+seg[i-1],LIM)] += dp[k-1][s];
                    }
                }
                for(int s= seg[i]+1; s<=LIM; s++)
                    ret+= dp[K-1][s];
            }
            return ret;
        }
