...
class UnLinker
{
    public:
        string clean(string text)
        {
            ...
            {
                //if(i>text.size() && mode==0) break;
                //cout<<i<<endl;
                //cout<<ret<<endl;
                if(i==text.size() && mode == 1 && )
                {
                    ret += text.substr(yin,text.size()-yin);
                    break;
                }
                if(mode ==0 )
                {
                    if(i+7<=text.size() && text.substr(i,7)== "http://")
                    {
                        yin=i;
                        i+=7;
                        mode=1;
                    }
                    else if(i+11 <= text.size() && text.substr(i,11) =="http://www.")
                    {
                        yin=i;
                        i+=11;
                        mode = 1;
                    }
                    else if(i+4 <= text.size() && text.substr(i, 4) =="www.")
                    {
                        yin =i;
                        i+=4;
                        mode =1;
                ...
                else
                {
                    if(mode ==1 )
                    {
                        if( i==text.size() || text[i]==' ' ||text[i]=='/' || text[i]==':' || text[i]==',')
                        {
                            if(text.substr(recent,4)==".com" || text.substr(recent, 4) == ".org" || text.substr(recent, 4)==".edu")
                            {
                                ret+="OMIT"+toString(num); 
                                num++;
                                mode =0;
                                i = recent +4;
                            }
                            else if(text.substr(recent,5)==".info")
                            {
                                ret+="OMIT"+ toString(num);
                                num++;
                                mode =0;
                        ...
                        {
                            if(i+4<=text.size() && ( text.substr(i, 4)==".com" || text.substr(i,4)==".org" || text.substr(i,4)==".edu"))
                            {
                                recent = i;
                                i+=4;
                            }
                            else if(i+5<=text.size() && text.substr(i,5)==".info")
                            {
                                recent =i; 
                                i+=5;
                            }
                            else if(i+3 <= text.size() && text.substr(i, 3) ==".tv")
                            {
                                recent =i;
                                i+=3;
                            }
