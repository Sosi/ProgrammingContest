...
{
        public:
        string newMember(vector <string> ex, string newName)
        {
            set<string> C;
            for(int i=0; i<ex.size(); i++) C.insert(ex[i]);
            if(C.find(newName)==C.end()) return newName;
            for(int i=1; i<51; i++)
            {
                string temp = newName +toString(i); 
                if(C.find(temp)==C.end()) return temp;
            }
            return newName;
        }
