...
class StonesGame
{
        ...
        {
			bool R=false;
			if(M%2==L%2 && K%2==1 &&abs(L-M)<=K)
				R=true;
			if(M%2!=L%2 && K%2==0 && abs(L-M)<=K)
				R=true;
			vector<int > P;
			for(int i= max(M-K+1,1);i<=M;i++)
			{
				if(i+K-1>N)
					continue;
				P.push_back(K+i-1-M+i);
			}
			if(R) return "Romeo";
			bool S=true;
			...
			{
				int m=P[i];
				if((m%2==L%2 && K%2==1 &&abs(L-m)<=K) || (m%2!=L%2 && K%2==0 && abs(L-m)<=K));
				else
					S=false;
			}
			if(S)
