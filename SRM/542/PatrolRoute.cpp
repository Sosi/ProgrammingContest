...
class PatrolRoute
{
        ...
        {
            long long ret=0LL;
            for(long long IX=4; IX<=2*(X-1); IX+=2)
            {
                long long numIX = X-1-IX/2+1;
                int minY = max(4LL, minT-IX);
                if(minY%2!=0) minY++;
                int maxY = min(2LL*(Y-1),maxT - IX);
                if(maxY%2!=0) maxY--;
                if(maxY < minY) continue;
                else 
                {
                    for(long long IY=minY; IY<=maxY; IY+=2LL)
                    {
                        long long numIY = Y-1-IY/2+1;
                        ret += numIX * numIY *(IX/2-1)* (IY/2-1)*6LL;
                    }
                }
                ret %=1000000007;
            }
