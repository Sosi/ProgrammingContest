...
string ff(string a, char b)
{
    ...
    else 
    {
        ...
        sort(c.begin(), c.end());
        for(int i=c.size()-1; i>=0; i--) ret+=c[i];
        return ret;
    }

}
string CM(string a, string b)
{
        ...
        string maxNumber(vector <int> matches, int n)
        {
            ...
            for(int i=1; i<55; i++)
            {
                for(int j=0; j<m; j++)
                {
                    if(i>= matches[j])
                    {
                        string temp = ff( C[i-matches[j]], '0'+j);
                        C[i]= CM(C[i], temp);
                    }
                }
            }
            string ret;
            for(int i=1; i<=n; i++) ret = CM(ret, C[i]);
            return ret;
        }
