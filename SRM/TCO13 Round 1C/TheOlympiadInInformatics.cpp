...
// END CUT HERE
#line 5 "TheOlympiadInInformatics.cpp"
#include <vector>
#include <list>
#include <map>
#include <set>
...
class TheOlympiadInInformatics
{
    public:
        int find(vector <int> sums, int k)
        {
            ...
            {
                int MAX = -1;
                REP(i,sums.size()) MAX = max(MAX, sums[i]);
                return MAX;
            }
            long long sigma = accumulate(sums.begin(), sums.end(), 0LL);
            long long ret = sigma / k ;
            int low = 1; int high = ret;
            while( high-low > 1)
            {
                int mid = low + (high-low)/2;
                int sum = 0;
                REP(i,sums.size()) sum+= sums[i]/mid;
                if(sum > k){
                    low = mid;
                }else high = mid;
            }
