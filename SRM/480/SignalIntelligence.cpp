...
// END CUT HERE
//Start with an infinite String consisting solely of '0' characters.
//The characters within the string are indexed from left to right 1, 2, 3, ... .
//
//For each input integer X in the signal, Gogo will allocate it within the string.
//To do this, he will select an index 2^p, where p is a non-negative integer that he may choose arbitrarily, 
//but in such way that characters at indices between 2^p and 2^p + X - 1, inclusive, are all equal to '0'.
//The allocating procedure consists of replacing all these characters with '1's. 
//This newly created sequence of consecutive '1' characters is said to represent the allocated integer X.
//In order to allow unique decryption, the following property must hold 
//after all numbers from the signal are allocated. 
//For every two integers from the input signal, there must be at least one '0' character 
//between the sequences of '1's that represent these integers.

//After all numbers are allocated within the string properly,
//the '1' character with the largest index is found
//and all '0' characters to the right of it are removed.
