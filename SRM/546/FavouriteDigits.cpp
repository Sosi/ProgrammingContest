...
class FavouriteDigits
{
        ...
        {
            int num=0; long long temp =N;
            while(temp>0) 
            { temp/=10; num++;} cout<<"num "<<num<<endl;
            temp =N; int tempnum=0;
            string cc;
            while(tempnum<count1+count2)
            {
                if(tempnum<=count1+count2) cc+=temp%10+'0';
                temp/=10;
                tempnum++;
            }
            reverse(cc.begin(),cc.end());
            cout<<"cc "<<cc<<endl;;
            vector<int> C(count1+count2);
            for(int i=0;i<count1; i++) C.push_back(digit1);
            for(int i=0;i<count2; i++) C.push_back(digit2);
            vector<string> P;
            num=0;
            do
            {
                num++;
                string p;
                for(int i=0;i<C.size(); i++) p+=C[i]+'0';
                P.push_back(p);
