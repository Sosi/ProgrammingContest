...
{
        public:
        int r[100],G[100],S[100],B[100];
        vector<int> t;
        vector <int> findBestResult(vector <string> teams)
        {
            int n= teams.size();
            int best=1000, a=1,b=1,c=1;
            for(int i=0; i<n; i++)
            {
                istringstream in(teams[i]);
                in>>G[i]>>S[i]>>B[i];
            }
            int tot,s,tem;
            for(int i=1; i<=1000; i++)
            {
                for(int j=1; j<=i; j++)
                {
                    for(int s=0; s<n; ++s) r[s]=G[s]*i+S[s]*j;
                    t.clear(); t.push_back(1);t.push_back(j);
                    for(int s=1; s<n; s++)
                    {
                        if(!(r[s]<=r[0]&& B[s]<=B[0]|| r[s]>=r[0]&&B[s]>=B[0]))
                        {
                            if(r[s]<r[0]) tem =(r[0]-r[s])/(B[s]-B[0]),t.push_back(tem-1),t.push_back(tem),t.push_back(tem+1);
                            else
                                tem=(r[s]-r[0])/(B[0]-B[s]),t.push_back(tem),t.push_back(tem-1),t.push_back(tem+1);
                        }
                    }
                    sort(t.begin(),t.end());
                    ...
                    for(int k=0; k<t.size(); k++)
                    {
                        if(t[k]>0 && t[k]<=j)
                        {
                            ...
                            for(int s=0; s<n; ++s)
                            {
                                r[s]=G[s]*i+S[s]*j+B[s]*t[k];
                                if(r[s]>r[0]) tot++;
                            }
                            if(tot<best) best=tot, a=i,b=j,c=t[k];
                        }
                    }
                }
            }
