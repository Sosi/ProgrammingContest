...
class DucksAlignment
{
        ...
        int minimumTime(vector <string> grid)
        {
            ...
            //cout<<C.size()<<endl;
//            for(int i=0; i<C.size(); i++) cout<<C[i].A<<" "<<C[i].B<<endl;
            //cout<<1<<endl;
            int N=100000000;
            for(int i=0; i<grid.size(); i++) //row
            {
                ...
                {
                    res+=abs(C[j].A-i); K.push_back(C[j].B);
                }
                cout<<"res  "<<res<<endl;
                sort(K.begin(),K.end());
                int M =100000000;
                //cout<<2<<endl;
                //cout<<grid[0].size()-C.size()<<endl;
                for(int j=0; j<= (int)grid[0].size()-(int)K.size(); j++)
                {
                    int tempres=0;
                    for(int m=0; m<K.size(); m++)
                        tempres+=abs(K[m]- j- m);
                    //cout<<"j  "<<j<<"  tempres "<<tempres<<endl;
                    M=min(tempres,M);
                }
                N=min(N,res+M);
            }
            ...
            for(int i=0; i<grid[0].size(); i++)
            {
                vector<int >K;
                int res=0;
                for(int j=0; j<C.size(); j++) res+=abs(C[j].B-i), K.push_back(C[j].A);
                sort(K.begin(),K.end());
                int M=1000000000;
                for(int j=0; j<=(int)grid.size()-(int)C.size(); j++)
                {
                    int tempres=0;
                    for(int k=0; k<K.size(); k++) tempres+=abs(K[k]-j-k);
                    M=min(tempres,M);
                }
                Q=min(Q,res+M);
            }
