...
class CheatingQuiz
{
        ...
        vector <int> howMany(string answers)
        {
            ...
            {
                if(answers[i]-'A'==0)A++; else if(answers[i]-'B'==0) B++; else C++;
            }
            vector<int> res;
            for(int i=0; i<answers.size(); i++)
            {
                if(A+B+C== max(C,max(A,B))) res.push_back(1); else if(min(A,min(B,C))>0) res.push_back(3);
