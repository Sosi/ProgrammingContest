...
public:
	...
	{
		int length=ballSequence.size();
		int K=length* repetitions;
		int begin=0,end=K-1;
		int revnum=0;    //代表逆序的数目   
		int count=1;     //count^1    count 为1 为正序 count 为0 为逆序
		int res=0;
 		while(begin!=end+1)
		{
			if(count)  
			{
				if(ballSequence[begin%length]=='B'&& revnum%2==0 ||(ballSequence[begin%length]=='W'&& revnum%2==1) )
				{
					res++;
					begin++;
					revnum=revnum^1;
			...
			{
				if(ballSequence[end%length]=='B'&& revnum%2==0 ||(ballSequence[end%length]=='W'&& revnum%2==1) )
				{
					res++;
					end--;
					revnum=revnum^1;
