...
public:
	...
	{
		int n = choice1.size();
		vector<int> ret( choice1.size(),1);
		vector<int> vis1(choice1.size()); vector<int> vis2(choice2.size());
		...
		while(1)
		{
			if(row == 0 && vis1[col] || row == 1 && vis2[col])
			{
				int p = 1;
				for(int i=0; i<n; i++)
				{
			...
			cout<<"! "<<row<<" "<<col<<endl;
			if(row == 0) vis1[col] = 1; else vis2[col] = 1;
			if(row == 0)
			{
				int target = choice1[col];
				int flag = 1;
				// search the opposite
				for(int j=0; j<n; j++)
				{
					if(choice2[j] == target && !vis2[j])
					{
						vis2[j] = 1; // vis1[j] = 1;
						ret[j] = 1;
						row = 0;
				...
				if(flag)
				{
					for(int j=0; j<n; j++)
					{
						if(!vis1[j] && j!= col && choice1[j] == target && !vis2[j])
						{
							vis2[j] = 1;
							ret[j] = 2;
							swap(choice1[j], choice2[j]);
							row = 0;
							col = j;
							flag = 0;
							break;
			...
			{
				// row = 1;
				int target = choice2[col];
				int flag = 1;
				// search the opposite
				for(int j=0; j<n; j++)
				{
					if(choice1[j] == target && !vis1[j])
					{
						vis1[j] = 1;
						ret[j] = 1;
						row = 1;
				...
				if(flag)
				{
					for(int j=0; j<n; j++)
					{
						if(!vis1[j] && j!= col && choice2[j] == target && !vis2[j])
						{
							vis1[j] = 1;
							ret[j] = 2;
							swap(choice1[j], choice2[j]);
